...
<body class="bg-gray-100">
    <div class="container mx-auto mt-8 p-8 grid grid-col-4 justify-content-center">
        <div class="col-span-4 text-center ">
            ...
            <form action="{{ route('categories.update',$category->id) }}" method="post" class="max-w-md mx-auto bg-white p-6 rounded-md shadow-md text-left" enctype="multipart/form-data">
                @csrf
                @method('put')
                <div class="mb-4">
                ...
                <div class="flex items-center justify-end gap-3">
                    <a href="{{route('categories.index')}}" class="bg-blue-500 text-white py-2 px-4 rounded">{{__('Back')}}</a>
                    <button type="submit" class="bg-blue-500 text-white py-2 px-4 rounded">{{__('Update')}}</button>
                </div>
            </form>
        </div>
    </div>
