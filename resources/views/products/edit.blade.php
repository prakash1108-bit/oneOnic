...
<body class="bg-gray-100">
    <div class="container mx-auto mt-8 p-8 grid grid-col-4 justify-content-center">
        <div class="col-span-4 text-center ">
            ...
            <form action="{{ route('product.update',$product->id) }}" method="post" class="max-w-md mx-auto bg-white p-6 rounded-md shadow-md text-left" enctype="multipart/form-data">
                @csrf
                @method('put')
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <select id="category" name="subcategory_id"
                        ...
                        @foreach ($subcategory as $item)
                            <option value="{{$item->id}}" {{$product->subcategory_id == $item->id ? 'selected' : ''}}>{{$item->name}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-4">
